...
test("renders and operates the student workbench across breakpoints", async ({ page }) => {
  ...
  try {
    ...
    await sql`
      ...
      values (${createdUser.id}, now() + interval '58 days', 14, ${sql.json({
        writing: 13,
        speaking: 13,
      })}, 600, true)`;
    ...
    const dashboardEstimates = [
      ["listening", 13, 12, 14, 0.82, "关键词定位较稳。", ["提升同义替换识别"]],
      ["reading", 12, 11, 13, 0.78, "长难句和段落功能判断是主要瓶颈。", ["训练判断题定位"]],
      ["writing", 11, 10, 12, 0.76, "观点明确，但论证链条和语法控制仍不稳定。", ["补因果链展开"]],
      ["speaking", 12, 11, 13, 0.74, "回答结构可理解，但 Part 3 抽象表达需要积累。", ["积累意群"]],
    ] as const;
    for (const [skill, estimated, low, high, confidence, summary, priorities] of dashboardEstimates) {
      await sql`
    ...
    await expect(page.getByText("写作最新阶段估分 5.5，目标 6.5")).toBeHidden();

    await page.getByRole("tab", { name: "写作" }).click();
    await expect(page.getByText("写作最新阶段估分 5.5，目标 6.5")).toBeAttached();
